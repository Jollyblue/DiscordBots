...
async def huntsplit(ctx, name: str=None, amount: str=None):
    ...
    if ctx.message.author.id not in mention_list:
        await bot.say('You forgot to mention yourself!  Try again.')
        return
...
async def sailors(ctx):
    ...
    players_list = ''
    cash_list = ''
    for place, entry in leaderboard[:30]:
...
async def wizze():
    """The banned scrub"""
    await bot.say(f'He just could not help himself and got banned, now Jayden has to learn Python. Damn him.')
    return
